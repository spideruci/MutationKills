...
print("failing test cases " + str(sum(result2)))
    ...
    if mutation["status"]=="killed":
        for test_case in mutation["test_cases"]:
            if test_case["state"]=="fail":
                ...
                if "org.apache.xmlgraphics" in test_case["Exception"]:
                    test_case["defensive"]=True
                    continue
                if "junit" in test_case["Exception"][0] or "mockito" in test_case["Exception"][0]:
                    test_case["assertion_failure"]=True
                    continue
                if len(test_case["record"][-1])>1 and test_case["record"][-1][1].startswith("org.junit"):
                    test_case["assertion_failure"]=True
                ...
                for r in test_case["record"][1:-1]:
                    ...
                    if stack_line_num ==throw_line_num and stack_file == throw_file and throw_exception ==stack_exception:
                        if not _.startswith("EXTest"):
                            ...
                            test_case["assertion_failure"]=True   
                        break
                if test_case["Exception"][0]=="java.lang.AssertionError" and test_case["assertion_failure"]==False:
...
for mutation in mutated_info:
    if mutation["status"] == "survive":
        if mutation["mutation_info"]["des"]=="removed call to java/io/PrintStream::println":
            mutation["status"] = "extra"
            


# 305 additional mutants
...
with open("xmlgraphics.csv", 'w', encoding='UTF8') as f:
    writer = csv.writer(f)
    # write the header

...
# all failing test has one exception
temp_set = set()
for mutation in mutated_info:
    if mutation["status"]=="killed":
        for test_case in mutation["test_cases"]:
    ...
    if mutation["status"]=="killed":
        for test_case in mutation["test_cases"]:
            if test_case["state"]=="fail":
                ...
                if test_case["Exception"].startswith("org"):
                    test_case["assertion_failure"]=True
                record = test_case["record"]
                
                # manually checked that AssertionError is from AssertJ
                if test_case["Exception"]=="java.lang.AssertionError" and test_case["record"][-1][1].startswith("org.apache.commons.text"):
                    test_case["assertion_failure"]=True
...
with open("commons-text.csv", 'w', encoding='UTF8') as f:
    writer = csv.writer(f)
    # write the header

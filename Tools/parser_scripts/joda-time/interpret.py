...
for line_num, line in enumerate(Lines()):
    ...
    if EX:
        if len(records) > 0 and not isinstance(records[-1][-1],list):
            if isinstance(records[-1][-1],list): continue
    ...
    if mutation["status"]=="killed":
        for test_case in mutation["test_cases"]:
            if test_case["state"]=="fail":
                ...
                if test_case["Exception"].startswith("junit"):
                    test_case["assertion_failure"]=True
                # no exception instantiation
                if test_case["Exception"].startswith("org.joda"):
...
with open("joda-time.csv", 'w', encoding='UTF8') as f:
    writer = csv.writer(f)
    # write the header

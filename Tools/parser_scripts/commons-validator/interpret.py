...
# veriy the number of killing tests are the same as what Pit reported
...
exceptions
# junit
# java.lang.AssertionError  are all from junit others are from source code


# <!-- check out AssertionError -->
    ...
    if mutation["status"]=="killed":
        for test_case in mutation["test_cases"]:
            if test_case["state"]=="fail":
                ...
                if "junit" in test_case["Exception"][0]:
                ...
                if test_case["Exception"][0]=="java.lang.AssertionError":
                    print("should print nothing here")
                    
                stack_trace = test_case["record"][-1]
                if len(stack_trace)==1:
                    print("there exists empty stack trace")
                    continue
                ...
                for r in test_case["record"][1:-1]:
                    if isinstance(r, list):
                        print("record one more exceptions")
                        continue
                    throw_source = r
...
plt.savefig("commons-validator" +".png")
#     plt.tight_layout()
plt.clf()
...
with open("commons-validator.csv", 'w', encoding='UTF8') as f:
    writer = csv.writer(f)
    # write the header
